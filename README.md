...
1.  **Clone the repository:**
    ...
    ```
    *(Note: adjust URL if necessary based on your actual repository URL).*
...
3.  **Run the test suite:**
    ...
    npm test
    ```
    *(Note: This executes the `run-tests.sh` bash script, triggering independent test runs for each endpoint).*
